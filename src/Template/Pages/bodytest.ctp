...
var typeD = ['Do your hands or feet get hot?','Do your body and face get hot?','Do you have dry skin or dry lips?','Is your natural lip color more red than others?','Are constipated easily?','Are you cheeks pink or red?','Do your eyes get dried?','Do you feel thirsty and need water?'];
...
function calcResult(){
  ...
  scores.forEach(function(score, i){
    ...
    $(bctQ).each(function(index){
      var subbctQ = "bctQ"+(index+1);
      if($(this).find("input[name='"+subbctQ+"']:checked").length !== 0){
        score += parseInt($(this).find("input[name='"+subbctQ+"']:checked").val());
      }else{
        score += 0;
      }
    });
  ...
  var maxVal = String(Math.max.apply(Math, scores));
